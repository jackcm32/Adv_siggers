...
void gainestimateRLS(float in1, float in2, float out, float gain[2])
{
    ...
    static float theta_hat1 = 0, theta_hat2 = 1.0, p11 = 0, p12 = 0, p21 = 0, p22 = 0;
    ...
    float p12_in2 = p12 * in1;
    float p21_in1 = p21 * in1;
    float p21_in2 = p21 * in2;
    float p22_in1 = p22 * in1;
    float p22_in2 = p22 * in2;

    float inv_lambda = 1 / lambda;

    float denom = (lambda + in1 * (p11_in1 + p21_in2) + in2 * (p12_in1 + p22_in2));

    float p11_new = inv_lambda * (p11 - (p11_in1 * (p11_in1 + p12_in2) + p21_in2 * (p11_in1 + p12*in2)) / denom);
    float p12_new = inv_lambda * (p12 - (p12_in1 * (p11_in1 + p12_in2) + p22_in2 * (p11_in1 + p12*in2)) / denom);
    float p21_new = inv_lambda * (p21 - (p11_in1 * (p21_in1 + p22_in2) + p21_in2 * (p21_in1 + p22*in2)) / denom);
    float p22_new = inv_lambda * (p22 - (p12_in1 * (p21_in1 + p22_in2) + p22_in2 * (p21_in1 + p22*in2)) / denom);
