...
G = [0;c1;0];
V = [0 0 0; 0 (sigma2_d + sigma2_d_4) 0; 0 0 sigma2_d];
H = [0 0 1];
W = [1];
...
q_out = 0.01;  % Chosen by setting input = low and checking output doesn't go below 0;
...
for t = 1:T_FINAL    
    [y(t), h(:,:,t+1)] = simulate_system_outflow( F, G, H, W, h(:,:,t), input(t), mu, sigma2_d, sigma_w, q_out, sigma2_d_4 );
...
plot_system( h, y, T_FINAL, input, z1, 0 )
